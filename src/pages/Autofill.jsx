...
function ManualLoginForm({ repo, dec, onClose, onSaved }) {
  ...
  const readImage = (file) => {
    ...
    if (!file.type.startsWith('image/')) {
      setFormError('Choose an image file for the local profile photo.');
      return;
    }
    if (file.size > MAX_AVATAR_BYTES) {
      setFormError('Choose an image smaller than 750 KB.');
      return;
    }
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (!isCompleteAutofillLogin(normalized)) {
        setFormError('Identity, password, and an exact website or Android package are required.');
        return;
      }
      ...
      setFormError('Could not save this login. Your entries were kept; please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <section className="mt-6 rounded-2xl border border-[#C8A96B]/35 bg-white/[0.03] p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="font-medium text-white">Add Autofill login</h2>
          <p className="mt-1 text-xs text-[#AEB4BE]">Link one permanent login to one exact website or Android app.</p>
        </div>
        <button type="button" onClick={onClose} aria-label="Close login form" className="rounded-lg p-2 text-[#AEB4BE] hover:text-white"><X className="h-4 w-4" /></button>
      </div>

      <form onSubmit={submit} className="mt-5 space-y-4">
        <div className="grid grid-cols-[8.5rem_1fr] gap-3">
          <div>
            <label htmlFor="autofill-identity-type" className="text-xs uppercase tracking-widest text-[#AEB4BE]">Identity type</label>
          ...
          <div>
            <label htmlFor="autofill-identity" className="text-xs uppercase tracking-widest text-[#AEB4BE]">Identity</label>
            ...
            {touched.identity && missingIdentity ? <p className="mt-1 text-xs text-red-400">Enter an email, username, or phone number.</p> : null}
          </div>
        </div>

        <div>
          <label htmlFor="autofill-password" className="text-xs uppercase tracking-widest text-[#AEB4BE]">Password</label>
          ...
          {touched.password && missingPassword ? <p className="mt-1 text-xs text-red-400">Enter a permanent password or login code.</p> : null}
        </div>

        <div className="grid grid-cols-[8.5rem_1fr] gap-3">
          <div>
            <label htmlFor="autofill-target-type" className="text-xs uppercase tracking-widest text-[#AEB4BE]">Target type</label>
          ...
          <div>
            <label htmlFor="autofill-target" className="text-xs uppercase tracking-widest text-[#AEB4BE]">{values.targetType === 'website' ? 'Domain' : 'Package name'}</label>
            ...
            {touched.target && missingTarget ? <p className="mt-1 text-xs text-red-400">Enter an exact domain or Android package.</p> : null}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-3">
          <label className="flex cursor-pointer items-center gap-3 text-sm text-[#AEB4BE]">
            ...
            <span>{values.image ? 'Replace selected local photo' : 'Choose a local profile photo (optional)'}</span>
            <input type="file" accept="image/*" className="sr-only" onChange={(event) => readImage(event.target.files?.[0])} />
          </label>
          {values.image ? <button type="button" onClick={() => set('image', '')} className="mt-2 text-xs text-[#C8A96B]">Remove selected photo</button> : null}
        </div>
...
export default function Autofill() {
  ...
  const importSavedLogins = async () => {
    ...
    try {
      ...
      setError('Could not import saved logins. Your existing vault data is unchanged; please try again.');
    } finally {
      setImporting(false);
    }
  };
    ...
    <div className="px-5 pb-32 pt-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-[#AEB4BE]">Store and manage sign-ins for websites and Android apps.</p>
        </div>
      ...
      <section className="mt-6 rounded-2xl border border-[#C8A96B]/30 bg-gradient-to-br from-[#C8A96B]/10 to-white/[0.03] p-4">
        <div className="flex gap-3">
          <div className="rounded-xl bg-[#C8A96B]/15 p-3 text-[#C8A96B]"><CheckCircle2 className="h-5 w-5" /></div>
          <div className="min-w-0 flex-1">
            <p className="font-medium text-white">Autofill save is ready</p>
            <p className="mt-1 text-sm text-[#AEB4BE]">After a supported login, approve Android&apos;s save prompt, then import it here.</p>
          </div>
        </div>
        <button type="button" onClick={importSavedLogins} disabled={importing} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#C8A96B]/50 px-4 py-3 text-sm font-medium text-[#C8A96B] disabled:opacity-60">
          ...
          {importing ? 'Importing saved logins…' : 'Import saved login'}
        </button>
      </section>

      <div className="relative mt-6">
        <Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#AEB4BE]" />
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search saved logins" className="w-full rounded-2xl border border-white/10 bg-white/[0.04] py-3 pl-11 pr-4 text-sm text-white outline-none placeholder:text-[#AEB4BE] focus:border-[#C8A96B]/60" />
      </div>
      ...
      <div className="mt-4 space-y-3">
        {!loading && filtered.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/15 bg-white/[0.03] p-6 text-center">
            ...
            <p className="mt-2 text-sm text-[#AEB4BE]">Save one manually, or approve Android&apos;s Autofill save prompt after signing in.</p>
            <button type="button" onClick={() => setAddOpen(true)} className="mt-5 inline-flex items-center gap-2 rounded-xl border border-[#C8A96B]/50 px-4 py-2 text-sm text-[#C8A96B]"><Plus className="h-4 w-4" />Save a login</button>
          </div>
        ) : (
          filtered.map(({ item, profile, visibleTitle }) => {
            const ready = isCompleteAutofillLogin(item);
            const targetBadge = profile.website ? 'Website' : profile.applicationIdentifier ? 'Android app' : 'Needs identity and target';
            return (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3 text-xs text-[#AEB4BE]">
                    <span className="inline-flex min-w-0 items-center gap-2"><UserRound className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{profile.identity || 'No email, username, or phone saved'}</span></span>
